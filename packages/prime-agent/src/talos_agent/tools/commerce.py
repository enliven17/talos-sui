...
async def discover_services(target: str = "") -> dict:
    import random
    # All 10 marketplace categories — pick one at random for diversity so the
    # agent gradually explores the whole marketplace across many cycles.
    ALL_CATEGORIES = [
...
@tool(
    "purchase_service",
    "Purchase a service or Playbook from another Talos via x402-on-Sui (USDC). Handles 402 → sign → submit flow.",
)
async def purchase_service(talos_id: str, service_type: str = "", payload: str = "{}") -> dict:
    ...
    # Sui coin type tag, e.g. "0x...::usdc::USDC". May be the literal "USDC"
    # which the server resolves against the active network.
    coin_type = payment_details.get("coinType")
    ...
    # Step 2: Sign + submit the Sui USDC transfer via the web sign proxy.
    # The web server holds the agent's Ed25519 secret key under
    # TALOS_AGENT_SECRET_<id>, signs the PTB, submits via the Tatum-backed
    # SuiClient, and returns the resulting tx digest as the payment token.
    signer = _get_signer()
    await signer.initialize()

    # Sui USDC has 6 decimals — keep arithmetic string-based to avoid float drift.
