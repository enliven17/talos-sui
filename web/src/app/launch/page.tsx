...
import {
  ...
  TALOS_NAME_SERVICE_PACKAGE_ID,
} from "@/lib/sui-move";
import { storeJsonOnWalrus } from "@/lib/walrus";
import { usdcToMicros, isValidSuiAddress } from "@/lib/sui";
...
export default function LaunchPage() {
  return (
    <WalletGate
      ...
      description="Creating a TALOS requires a Sui wallet connection. Your address will be registered as the Creator."
    >
      <LaunchForm />
    </WalletGate>
  );
...
function LaunchForm() {
  ...
  const handleLaunch = async () => {
    ...
    const creatorAddr = (form.creatorWallet || address) as string;

    try {
      // 1. Push extended profile metadata to Walrus first — its blob id will
      //    be stored inside the on-chain Talos object so anyone can fetch the
      //    rich profile (description, persona, etc.) without going through us.
      setDeployStep("Storing profile metadata on Walrus...");
      setDeployProgress(1);
      try {
        const blob = await storeJsonOnWalrus({
          ...
          mitos: {
            ...
            initialPriceUsd: form.initialPrice,
          },
        ...
        console.warn("[Launch] Walrus store failed, continuing without blob:", e);
      }

      // 2. Register TALOS on-chain via Sui Move (if package is configured)
      if (TALOS_REGISTRY_PACKAGE_ID) {
        setDeployStep("Registering TALOS on Sui...");
        setDeployProgress(2);

        const treasuryAddr = creatorAddr; // treasury defaults to creator until governance moves it
        const investorAddr = creatorAddr; // single-creator launch — investor seat defaults to self
        const tx = buildCreateTalosTx({
          name: form.productName,
          category: form.category.charAt(0).toUpperCase() + form.category.slice(1),
          description: form.persona || form.productDesc,
          creatorShareBps: 0,
          investorShareBps: 0,
          treasuryShareBps: 10_000, // 100% to treasury initially
          investorAddr,
          treasuryAddr,
          approvalThresholdMicros: usdcToMicros(form.approvalThreshold || "10"),
          gtmBudgetMicros: usdcToMicros(form.gtmBudget || "100"),
          minPatronPulse: BigInt(Math.floor(Number(form.totalSupply) / 1000) || 100),
          totalSupply: BigInt(form.totalSupply || "1000000"),
          priceUsdMicros: usdcToMicros(form.initialPrice || "0.01"),
          tokenSymbol: form.tokenSymbol.toUpperCase(),
          walrusProfileBlob: walrusBlobId ?? "",
        });

        const result = await signAndExecute(tx);
        console.log("[Launch] Sui tx digest:", result.digest);

        // Parse the TalosCreated event to recover the assigned talos_id.
        const createdEvent = result.events.find((e) =>
          e.type.endsWith("::registry::TalosCreated"),
        );
        if (createdEvent?.parsedJson && typeof createdEvent.parsedJson === "object") {
          const parsed = createdEvent.parsedJson as { talos_id?: string | number };
          if (parsed.talos_id != null) {
            onChainId = Number(parsed.talos_id);
          }
        }
        // Also capture the new shared Talos object id so the dashboard can
        // open SuiVision links and the server can record activity batches.
        const sharedTalos = result.objectChanges.find(
          (c) =>
            c.type === "created" &&
            "objectType" in c &&
            typeof c.objectType === "string" &&
            c.objectType.endsWith("::registry::Talos"),
        );
        if (sharedTalos && "objectId" in sharedTalos) {
          (form as Record<string, unknown>).onChainObjectId = sharedTalos.objectId;
        }
      } else {
        setDeployProgress(2);
      }

      // 3. Register agent name on-chain
      if (TALOS_NAME_SERVICE_PACKAGE_ID && onChainId !== null && onChainId > 0) {
        setDeployStep("Registering agent identity on-chain...");
        setDeployProgress(3);
        try {
          const tx = buildRegisterNameTx(
            BigInt(onChainId),
            form.agentName.toLowerCase().trim(),
          );
          const result = await signAndExecute(tx);
          console.log("[Launch] name register tx:", result.digest);
        } catch (nameErr) {
          // Name registration is non-fatal — Talos already exists on-chain
          console.warn("[Launch] Name registration failed:", nameErr);
        }
      } else {
        setDeployProgress(3);
      }
      ...
      try {
        const res = await fetch("/api/talos", {
          ...
          body: JSON.stringify({
            ...
            onChainObjectId: (form as Record<string, unknown>).onChainObjectId as string | undefined,
            agentName: form.agentName,
            tokenSymbol: form.tokenSymbol.toUpperCase() || undefined,
            walrusProfileBlob: walrusBlobId ?? undefined,
        ...
        const onChainInfo = onChainId !== null
          ? `On-chain registration succeeded (Walrus: ${walrusBlobId?.slice(0, 12) ?? "n/a"}…) but database save failed: ${msg}. `
      ...
      let message = raw;
      if (raw.includes("User declined") || raw.includes("user rejected") || raw.toLowerCase().includes("rejected"))
        message = "Transaction was rejected in your wallet.";
      else if (raw.toLowerCase().includes("insufficient"))
        message = "Insufficient SUI balance for this transaction.";
        ...
        <div className="bg-surface border border-accent/30 p-8 mb-6 space-y-6">
          {genesisResult.onChainId !== null && (
            <div className="flex items-center gap-3 text-accent font-bold">
              ...
              <span className="text-sm font-medium">Sui registration confirmed</span>
            </div>
          )}
          ...
          {genesisResult.walrusBlobId && (
            <div>
              <div className="text-xs text-muted mb-2">Walrus profile blob</div>
      ...
      <div className="bg-surface border border-border p-8 mb-6">
          ...
          <div className="space-y-6">
            ...
            <div className="pt-4 mt-2 border-t border-border">
              ...
              <p className="text-xs text-muted mb-4">
                Define the paid service your agent offers to other agents via x402-style payments on Sui. Optional — you can add this later.
              </p>
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              The Creator is your Sui address. It is permanently linked to this TALOS and cannot be changed.
            </p>
            <div>
              <label className="block text-xs text-muted mb-2">Creator Address (Sui)</label>
              ...
              <p className="text-xs text-muted mt-1">This Sui address will be registered as the TALOS Creator on-chain.</p>
              {address && !isValidSuiAddress(form.creatorWallet || address) && (
                <p className="text-xs text-red-500 mt-1">Connected wallet is not a valid Sui address.</p>
              )}
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              Configure your TALOS&apos;s Mitos token on Sui (Coin&lt;T&gt; module published separately).
            </p>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">2-8 characters, letters and numbers only. Used as the Move struct name for the Mitos Coin.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">Price per Pulse token in USDC on Sui</p>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              <ReviewRow label="Creator (Sui)" value={form.creatorWallet || address || ""} />
              ...
              <div className="text-xs text-muted pt-2 border-t border-border">
                Extended profile metadata is published to Walrus before the on-chain registration.
                Only the Walrus blob id is stored on Sui to keep gas costs low.
              </div>
            </div>
          </div>
        )}
